...
def EXTRA_plot_4_models(models, models_names):
    """
    Function to plot four (and only four!) climate models in a subplot. Using
    Plate-Carrée Porjections

    Keyword arguments:
    models -- dictionairy of the the model values and model names. Model value
                should be in global dimension lon/lat and contain
    ...
    for i, j in zip(axes, index):
        ...
        i.coastlines()
        data.plot.pcolormesh(ax=i, cmap='coolwarm', robust=True,
                             label="Temperatur",
                             cbar_kwargs={'label': "Temp. in K"})
        i.coastlines()
        i.set_extent([-160, 160, -90, 90])
        gl = i.gridlines(draw_labels=True)
        gl.xlabel_style = dict(fontsize=9)
        gl.ylabel_style = dict(fontsize=9, rotation=90, va='bottom',
                               ha='center')
        gl.top_labels = False
        gl.right_labels = False
        i.set_title("1970-2014 mean temperature {}".format(models_names[j]),
                    fontsize=16)


def EXTRA_diffs_to_era5(models, era5_mean, models_names):
    """
    Function to plot four (and only four!) climate models and their
    differences to the era5 mean in a subplot. Using  Plate-Carrée Porjections
    ...
    for i, j in zip(axes, index):
        ...
        i.coastlines()
        data.plot.pcolormesh(ax=i, cmap='coolwarm', robust=True,
                             cbar_kwargs={'label': "Temp. difference in K"})
        i.coastlines()
        i.set_extent([-160, 160, -90, 90])
        gl = i.gridlines(draw_labels=True)
        gl.xlabel_style = dict(fontsize=9)
        ...
        gl.top_labels = False
        gl.right_labels = False
        i.set_title("1970-2014 dTemp: of {} to ERA5".format(models_names[j]),
                    fontsize=13)
